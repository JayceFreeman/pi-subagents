...
describe("model fallback helpers", () => {
	...
	it("builds a deduplicated ordered candidate list", () => {
		assert.deepEqual(
			buildModelCandidates("gpt-5-mini", ["openai/gpt-5-mini", "anthropic/claude-sonnet-4", "gpt-5-mini"], availableModels, availableModels),
	...
	it("applies the current provider preference to fallback candidates too", () => {
		...
		assert.deepEqual(
			buildModelCandidates("gpt-5-mini", ["gpt-5-mini", "anthropic/claude-sonnet-4"], ambiguous, ambiguous, "github-copilot"),
...
describe("tryResolveModelCandidate", () => {
	const known: AvailableModelInfo[] = [
		...
		{ provider: "openai", id: "gpt-5-mini", fullId: "openai/gpt-5-mini" },
		{ provider: "anthropic", id: "claude-opus-4-7", fullId: "anthropic/claude-opus-4-7" },
		{ provider: "amazon-bedrock", id: "amazon.nova-lite-v1:0", fullId: "amazon-bedrock/amazon.nova-lite-v1:0" },
	];
	const available: AvailableModelInfo[] = known.slice(0, 2);

	it("preserves real colon-bearing model ids", () => {
		const result = tryResolveModelCandidate("amazon-bedrock/amazon.nova-lite-v1:0", known, available);
	...
	it("returns not-authed error when model in knownModels but not availableModels", () => {
		const result = tryResolveModelCandidate("anthropic/claude-opus-4-7", known, available);
		assert.equal(result.resolved, undefined);
		assert.match(result.error!, /not authed/);
	});

	it("suggests the nearest model id on a bare-id typo (suggestion-first)", () => {
		const result = tryResolveModelCandidate("openai/gpt-5-mimi", known, available);
		assert.equal(result.resolved, undefined);
	...
	it("suggests a valid thinking level for unrecognized colon-suffix on known base", () => {
		const result = tryResolveModelCandidate("openai/gpt-5-mini:hihg", known, available);
		assert.equal(result.resolved, undefined);
	...
	it("passes through an exact-known model unchanged (smoke gate case a)", () => {
		const result = tryResolveModelCandidate("openai-codex/gpt-5.5:xhigh", known, available);
	...
	it("surfaces unknown-model error for reported failure shape without login/auth fallback", () => {
		const result = tryResolveModelCandidate("openai/gpt-5.5-codex:xhigh", known, available);
		assert.equal(result.resolved, undefined);
		assert.match(result.error!, /Unknown model: 'openai\/gpt-5\.5-codex:xhigh'/);
...
describe("buildModelCandidates fallback semantics", () => {
	...
	it("drops invalid fallbacks but keeps the valid primary", () => {
		const candidates = buildModelCandidates("openai-codex/gpt-5.5:high", ["bogus/typo:high"], known, known);
		assert.deepEqual(candidates, ["openai-codex/gpt-5.5:high"]);
	});

	it("uses a valid fallback when the primary is unknown (no throw)", () => {
		const candidates = buildModelCandidates("bogus/typo:high", ["openai-codex/gpt-5.5:high"], known, known);
		assert.deepEqual(candidates, ["openai-codex/gpt-5.5:high"]);
	});

	it("throws aggregated suggestion-first error when every candidate is invalid", () => {
		assert.throws(
			() => buildModelCandidates("bogus/typo:high", ["another/wrong:low"], known, known),
